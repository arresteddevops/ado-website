...
<div class="wrap">
<?php echo PrliAppHelper::page_title(__('Edit Link Report', 'pretty-link')); ?>
...
<table class="form-table">
  <tr class="form-field">
    ...
    <td><input type="text" name="name" value="<?php echo ((isset($_POST['name']) and $record == null)?$_POST['name']:$record->name); ?>" size="75">
        ...
        <ul width="100%">
          ...
          for($i = 0; $i < count($links); $i++)
          {
                  ...
                  ( is_object($prev_link) and $prev_link->group_name != $link->group_name ) ) )
            {
              ...
              ?>
                <li width="100%"><span style="padding: 5px; margin: 0px; font-size: 14px; background-color: #ababab; display: block;"><input type="checkbox" style="width: 15px;" class="group-checkbox-<?php echo $link->group_id; ?>" name="group-row-<?php echo $link->group_id; ?>" <?php echo (((isset($_POST['group-row-'.$link->group_id]) and $_POST['group-row-'.$link->group_id] == 'on'))?'checked="true"':''); ?>/>&nbsp;<?php echo substr(stripslashes($link->group_name),0,50); ?></span>
        ...
        <table width="100%" cellspacing="0">
          ...
          for($i = 0; $i < count($links); $i++)
          {
            ...
            <tr <?php echo (($i%2)?' style="background-color: #efefef;"':''); ?>>
              <td style="padding: 5px; margin: 0px;" width="50%"><input type="radio" style="width: 15px;" name="goal_link_id" value="<?php echo $link->id; ?>" <?php echo (((isset($_POST['goal_link_id']) and $_POST['goal_link_id'] == $link->id) or (empty($_POST) and $link->id == $record->goal_link_id))?'checked="true"':''); ?>/>&nbsp;<?php echo substr(stripslashes($link->name),0,25) . " <strong>(" . $link->slug . ")</strong>"; ?></td>
...
<p class="submit">
<input type="submit" name="Submit" value="Update" />&nbsp;or&nbsp;<a href="?page=<?php echo PRLI_PLUGIN_NAME ?>/pro/prlipro-reports.php&action=list"><?php _e('Cancel', 'pretty-link'); ?></a>
</p>
